...
# Simple tracking
online_users: dict[int, str] = {}  # {user_id: socket_id}
user_matches: dict[int, str] = {}  # {user_id: room_id}
...
def init_match_events(socketio):
    ...
    @socketio.on("request_users_online")
    def handle_request_users_online():
        emit("users_online", {"count": len(online_users)})
    ...
    def on_connect():
        ...
        try:
            ...
            socketio.emit("users_online", {"count": len(online_users)})
        except:
            db.session.rollback()
    ...
    def add_friend(data):
        ...
        try:
            ...
            if current_user.is_friend_with(other):
                emit("status", f"Already friends with {other.username}")
                return

            # Check if request already exists
            from models.models import FriendRequest
            ...
            if existing_request:
                emit("status", "Friend request already sent!")
                return
                
            # Create a PROPER friend request
            ...
            other_socket = online_users.get(other_id)
            if other_socket:
            ...
            emit("status", f"✅ Request sent to {other.username}!")
            logger.info(f"📬 Friend Request: {user_id} -> {other_id}")
